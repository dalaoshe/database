...
//                        string colname1 = string((*(createStatement->columns))[i]->check_expr->expr->expr->name);
                        printf("colname: %s  \n",(*(createStatement->columns))[i]->check_expr->expr->name);
                        //获取操作
                        CompOp operate;
                        char op1 =(*(createStatement->columns))[i]->check_expr->op_char;
                        if(op1!=' '){
                            switch(op1){
                                case '>':{
                            ...
                            switch ((*(createStatement->columns))[i]->check_expr->op_type){
                        ...
                        //根据列名获取值的类型
                        AttrType type;
                        for(int i=0;i<attr->key_name.size();++i){
                            if(colname == attr->key_name[i]){
                                type = attr->key_type[i];
                                break;
                            }
                        }
                        if(operate!= CompOp::IN_OP){
                            ...
                            switch(type){
                                case INT:{
                                    value.int_data = (*(createStatement->columns))[i]->check_expr->ival;
                                    break;
                                }
                                case FLOAT: {
                                    value.float_data = (*(createStatement->columns))[i]->check_expr->fval;
                                    break;
                                }
                                case STRING: {
                                    value.string_data = ((*(createStatement->columns))[i]->check_expr->name);
                                    break;
                                }
                            }
                        }else{//
                            vector<Expr*> check_values = *(*(createStatement->columns))[i]->check_expr->literal_list;
                            vector<AttrValue> values;
                            int check_attr_size = check_values.size();
                            switch(type){
                                case INT: {
                                    for (int k = 0; k < check_attr_size; ++k) {
                                        AttrValue temp_value;
                                        temp_value.int_data= (*(createStatement->columns))[i]->check_expr->ival;
                                ...
                                case FLOAT:{
                                    for (int k = 0; k < check_attr_size; ++k) {
                                        AttrValue temp_value;
                                        temp_value.float_data= (*(createStatement->columns))[i]->check_expr->fval;
                                        values.push_back(temp_value) ;
                                    }
                                    break;
                                }
                                case STRING:{
                                    for (int k = 0; k < check_attr_size; ++k) {
                                        AttrValue temp_value;
                                        temp_value.string_data= check_values[k]->name;
                                        printf("%d %s\n",k,temp_value.string_data);
                                        values.push_back(temp_value) ;
                                    }
                                    for(int k=0;k<values.size();++k){
                                        printf("%s  \t",values[k].string_data);
                                    }
                                    break;
                                }
                            }
//                            for(int i=0;i<check_attr_size;++i){
//                                printf("%s\t",check_values[i]->name);
//                            }
                        }
                        continue;
                    }
